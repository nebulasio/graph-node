...
type GroupedFieldSet<'a> = IndexMap<&'a String, CollectedResponseKey<'a>>;
        ...
        for cond in &self.conds {
            ...
            if !parents_by_id.is_empty() {
                ...
                windows.push(EntityWindow {
                    ...
                    ids,
                    link,
                    // TODO: discover how to pass ColumnNames to this part.
...
impl<'a> CollectedResponseKey<'a> {
    fn collect_field(
        ...
        if is_reference_field(document, object_or_interface, field) {
            ...
            todo!("handle non-reference field (e.g.: scalar)");
        }
    }
}
...
fn is_reference_field(
    document: &s::Document,
    object_or_interface: ObjectOrInterface,
    field: &q::Field,
) -> bool {
    object_or_interface
        .field(&field.name)
